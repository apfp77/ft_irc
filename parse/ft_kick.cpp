...
static void check_parameter(std::vector<std::string> &recv_vector);
static Channel *check_exist_channel(std::string recv_vector, Server &serv);
...
void ft_kick(std::vector<std::string> &recv_vector, Client *cli, Server &serv)
{
	try
	{
		check_parameter(recv_vector);
		Channel *channel = check_exist_channel(recv_vector[1], serv);
	...
	for (siz_t i = 0; i < end;)
	{
		try{
			if (channel->find_cli_in_ch_by_str(users[i]) == NULL)
			{
				...
				err_box.err_message = "he isn't on that channel";
				throw (err_box);
			}
			i++;
		}
...
static void check_parameter(std::vector<std::string> &recv_vector)
{
	if (recv_vector.size() <= 2)
	{
		...
		err_box.err_message = "Not enough parameters";
		throw (err_box);
	}
}
...
static Channel *check_exist_channel(std::string recv_vector, Server &serv)
{
	...
	if (channel == NULL)
	{
		...
		err_box.err_message = "You're not on that channel";
		throw (err_box);
	}
	return (channel);
}
...
static void check_me_operator_in_channel(Channel *channel, Client *cli)
{
	if (channel->find_cli_in_gm_ch(cli) == NULL)
	{
		...
		err_box.err_message = "You're not channel operator";
		throw (err_box);
	}
}